...
public class GameHelper  implements GooglePlayServicesClient.ConnectionCallbacks,
	...
	private int REQUEST_ACHIEVEMENT = 222;
	
	private GameHelper.GameHelperListener mLoginListener;
	
    /**
	 * Unlock an achievement
	 * @param pAchievement
	 */
    public void unlockAchievement(String pAchievement){
		if(isSignedIn() == false)
			return;
		getGamesClient().unlockAchievement(pAchievement);
	}
    
    /**
	 * Start activity that displays achievements
	 */
	...
	/**
	 * Ads a new score to leadearboard
	 * @param pLeaderboard
	 * @param pScore
	 */
	public void submitScore(String pLeaderboard, long pScore){
		if(isSignedIn() == false)
			return;
		getGamesClient().submitScore(pLeaderboard, pScore);
	}
	...
	public void startLeaderboardActivity(String leaderboardId){
		if(isSignedIn())
    ...
    public void showSignOutDialog(){
    	if(mSignOutDialog == null){
			...
			mSignOutDialog = alert.create();
    	}
    	if(isSignedIn())
    		mSignOutDialog.show();
    }
    //***END ACHIEVEMENTS & LEADERBOARDS
    ...
    void notifyListener(boolean success) {
        ...
        if (mListener != null) {
            if (success) {
                ...
                //If leaderboard provided, once signed in, we start leaderboard activity
                if(mLeaderboardId != null)
        			mActivity.startActivityForResult(getGamesClient().getLeaderboardIntent(mLeaderboardId), REQUEST_LEADERBOARD);
            } else {
